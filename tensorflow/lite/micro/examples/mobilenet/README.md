...
```
make -f tensorflow/lite/micro/tools/make/Makefile TARGET=arc_emsdp OPTIMIZED_KERNEL_DIR=arc_mli ARC_TAGS=no_arc_mli generate_mobilenet_make_project
```
...
2.  Go to the generated example project director
    ...
    ```
    cd tensorflow/lite/micro/tools/make/gen/arc_emsdp_arc/prj/mobilenet/make
    ```
...
example near the bottom of the list named `TensorFlowLite:mobilenet`. Select
it and click `mobilenet` to load the example.
...
```
make -f tensorflow/lite/micro/tools/make/Makefile TARGET=esp generate_mobilenet_esp_project
```

### Building the example

Go to the example project directory `cd
tensorflow/lite/micro/tools/make/gen/esp_xtensa-esp32/prj/mobilenet/esp-idf`
...
```
make -f tensorflow/lite/micro/tools/make/Makefile generate_mobilenet_make_project TARGET=himax_we1_evb ARC_TAGS=no_arc_mli
```
...
1.  Go to the generated example project directory.
    ...
    ```
    cd tensorflow/lite/micro/tools/make/gen/himax_we1_evb_arc/prj/mobilenet/make
    ```
...
3.  After example build finish, copy ELF file and map file to image generate
    ...
    ```
    cp mobilenet.elf himax_we1_evb.map ../../../../../downloads/himax_we1_sdk/image_gen_linux_v3/
    ```
...
5.  run image generate tool, generate flash image file.
    ...
    *   Before running image generate tool, by typing `sudo chmod +x image_gen`
    ...
    ```
    image_gen -e mobilenet.elf -m himax_we1_evb.map -o out.img
    ```
...
```
make -f tensorflow/lite/micro/tools/make/Makefile TARGET=sparkfun_edge mobilenet_bin
```

The binary will be created in the following location:

```
tensorflow/lite/micro/tools/make/gen/sparkfun_edge_cortex-m4/bin/mobilenet.bin
```
...
--bin tensorflow/lite/micro/tools/make/gen/sparkfun_edge_cortex-m4/bin/mobilenet.bin \
--load-address 0xC000 \
--magic-num 0xCB \
...
```
make -f tensorflow/lite/micro/tools/make/Makefile TARGET=mbed TAGS="CMSIS disco_f746ng" generate_mobilenet_mbed_project
```

This will result in the creation of a new folder:

```
tensorflow/lite/micro/tools/make/gen/mbed_cortex-m4/prj/mobilenet/mbed
```
...
```bash
make -f tensorflow/lite/micro/tools/make/Makefile test_mobilenet_test
```
